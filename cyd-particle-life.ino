...
#define WORLD_W   320
#define WORLD_H   170
#define ORIGIN_Y  ((240 - WORLD_H) / 2)
#define HUD_Y     (ORIGIN_Y + WORLD_H)
#define HUD_H     (240 - HUD_Y)
...
void IRAM_ATTR __attribute__((optimize("O3"))) buildCellList() {
  ...
  numJobs[1] = 0;
  {
    ...
    const uint32_t NS_PER_CAND = 240;
...
void loop() {
  ...
  // 旧実装 vTaskDelayUntil(&frameWake, pdMS_TO_TICKS(1000/TARGET_FPS)) には
  // 2つの丸め誤差があった:
  //   1) 1000/TARGET_FPS は整数演算のため 16.67ms が 16ms に切り捨てられる
  //      → 1000/16=62.5fpsが実際の上限になる(ログで62.5が連続する形で観測済み)
  //   2) pdMS_TO_TICKS内部でも configTICK_RATE_HZ が1000Hzでない環境では
  //      さらに丸められ、環境によっては上限が70fps超まで外れてしまう
  // microsecond単位で直接比較することでtick分解能に依存しない正確な
  // 16667us(=60.0fps)周期を実現する。
  static uint32_t frameWakeUs = 0;
  const uint32_t periodUs = 1000000UL / TARGET_FPS;
  if (frameWakeUs == 0) frameWakeUs = micros();
  frameWakeUs += periodUs;
  int32_t remainUs = (int32_t)(frameWakeUs - micros());
  if (remainUs > 0) {
    delayMicroseconds(remainUs);
  } else if (-remainUs > (int32_t)periodUs) {
    // 大きく遅延した場合はペーシング基準を現在時刻へ再同期し、遅延の蓄積を防ぐ
    frameWakeUs = micros();
  }
}
